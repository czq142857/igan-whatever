...
class GUIDraw(QWidget):
	...
	def init_color(self, shadow):
		if shadow:
			...
			self.color = QColor(0, 0, 255)  # default color red
	...
	def update_msg(self, painter):
		...
		painter.drawText(QPoint(border, fontSz + border), (msg))
		...
		if num_frames > 0 and num_images > 0:
			...
			border = 3
			painter.setFont(QFont('Decorative', fontSz))
			painter.drawText(QPoint(border, 2 * fontSz + border), (msg))
	...
	def fix_z(self):
		...
		self.uiWarp.reset()
		self.update()
		self.opt_engine.init_constraints()

...
class Constrained_OPT(QThread):
	...
	def __init__(self, model, opt_solver, image_size=64, batch_size=32, dimz = 128, n_iters=25, topK=16, morph_steps=16, interp='linear'):
	...
	def run(self):  # main function
		time_to_wait = 50 # 50 millisecond
		while (1):
			t1 =time()
	...
	def update_invert(self, constraints):
		...
		#cv2.imwrite("x1.png", constraints[0])
		#cv2.imwrite("x2.png", constraints[1])
		#cv2.imwrite("x3.png", constraints[2])
		#cv2.imwrite("x4.png", constraints[3])
		
		#here I just stack the colored stroke directly on the original image
		#use predict_z to encode the combined image
		gx_t, z_t, cost_all = self.opt_solver.invert(constraints_c)
